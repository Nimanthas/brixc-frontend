...
module.exports = async (req, res) => {
  try {
    ...
    const { candidate_type, candidate_type_status, option } = req.body;
    const last_updated = moment().format('YYYY-MM-DD HH:mm:ss');

    let collection = client.db(settings.mongodb_name).collection("master_candidate_types"); // Update the collection name

    let result;

    if (option === 'insert' || option === 'update') {
      const filter = { candidate_type }; // Assuming location_id is a unique identifier
      const updateDoc = {
        $set: { candidate_type, candidate_type_status, last_updated }
      };
      ...
      if (!result) {
        ...
        result = {
          value: { candidate_type, candidate_type_status, last_updated },
        };
      }
      ...
      const deleteResult = await collection.findOneAndDelete({ candidate_type });
    ...
    res.status(200).json({ type: 'SUCCESS', message: `Candidate type ${option}ed successfully!`, data: result.value });
  } catch (error) {
