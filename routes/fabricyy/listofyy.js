...
module.exports = async (req, res) => {
  try {
    ...
    const { username = "", typeid = "" } = req.body;

    let sqlqry = "";

    // Check request type and set SQL query accordingly
    if (typeid === "ALL" && username) {
      ...
      sqlqry = `SELECT fabricyy_master.*, fabricyy_details.*, sys_customer.cus_name, sys_factory.fac_name 
                ...
                WHERE fabricyy_master.fabyy_status != 'Delete' AND fabricyy_master.username = '${username}' AND fabricyy_master.fabyy_id = '${typeid}'`;
    } else {
      throw new Error("Invalid request");
    }
