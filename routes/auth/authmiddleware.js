...
const authenticateToken = async (req, res, next) => {
  ...
  if (!authorization) {
    ...
    return res.status(200).json({ type: 'ERROR', error: 'authorization was not provided.', action: 'RETRY' });
  }
  ...
  if (!token) {
    // If not provided, send an error response
    return res.status(401).json({ type: 'ERROR', message: 'authorization token not provided.', action: 'RETRY' });
  }
