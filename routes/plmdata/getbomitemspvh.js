            ...
            for (var i = 0; i < itemlistids.length; i++)
            {
                ...
                if(letterNumber !== "" && itemlistids[i] !== "centric%3A")
                {
                    ...
                    if(resp.status === 200){
                        ...
                        else if(nameofmattype.node_name === "Sewing")
                        {
                            if(resp.data.node_name === "Tape" 
                            || resp.data.node_name === "Snap tape" 
                            || resp.data.node_name === "Neck tape" 
                            || resp.data.node_name === "Mobilon tape" 
                            || resp.data.node_name === "Elastic" 
                            || resp.data.node_name === "Zipper" 
                            || resp.data.node_name === "Buttons" 
                            || resp.data.node_name === "Snap")
                            {
                                items_fabric.push({id:itemlistids[i],
                                    actual:resp.data.actual,
                                    placement:resp.data.node_name,
